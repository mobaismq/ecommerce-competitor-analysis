...
While each product detail page is open, the runner opens 店透视 `商品图` / `商品图自定义下载`, verifies that only `1:1主图` and `详情图` are checked, clicks `按类型下载`, then moves and extracts only files created by that click into `product_page_images/main` and `product_page_images/detail`. It writes a `product_page_images.json` sidecar so `$mysql-import` can load those image assets into `product_page_image_asset` and `media_asset`. If this 店透视 export fails, the image step is marked failed; it must not scroll the page or use visible-page URL capture, because recommendation modules can contain images from other products.

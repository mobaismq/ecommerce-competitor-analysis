...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--allow-missing-sku", action="store_true", help="Allow missing SKU export and import product data with zero SKU rows.")
...
def main() -> int:
    ...
    if args.mode == "clean-and-load":
        ...
        sku_file_supplied = "--sku-file" in sys.argv
        sku_path = Path(args.sku_file).resolve() if args.sku_file and (sku_file_supplied or not args.allow_missing_sku) else None
        if sku_path and sku_path.exists():
            clean_cmd += ["--sku-file", str(sku_path)]
        elif args.allow_missing_sku:
            clean_cmd.append("--allow-missing-sku")
        else:
            clean_cmd += ["--sku-file", str(sku_path)]

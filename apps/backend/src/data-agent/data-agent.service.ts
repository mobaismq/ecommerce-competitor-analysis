...
export class DataAgentService {
  ...
  async chat(input: ChatInput) {
    ...
    const report = input.jobId
      ? await this.prisma.analysisRun.findUnique({ where: { jobId: input.jobId } })
      : await this.prisma.analysisRun.findFirst({ where: { tenantId: input.tenantId }, orderBy: { updatedAt: 'desc' } })
    const collection = input.jobId ? await this.prisma.collectionJob.findUnique({ where: { jobId: input.jobId } }) : null
    ...
    const result = await this.router.execute(
      ...
      { tenantId: input.tenantId, jobId: input.jobId, attemptKey },
    )

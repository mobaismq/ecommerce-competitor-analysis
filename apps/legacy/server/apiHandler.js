...
import { changeAccountPassword, checkAccountDuplicate, createAccount, deleteAccount, getAccountPermissions, listAccounts, loginAccount, updateAccount } from '../src/server/accountManagement.js'
...
function findBatchSummaryFile(run) {
  ...
  const root = path.resolve(APP_DIR, '..', 'rpa_runs')
...
export async function handleRequest(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET' && req.url.startsWith('/api/account/check-duplicate')) {
      const requestUrl = new URL(req.url, 'http://localhost')

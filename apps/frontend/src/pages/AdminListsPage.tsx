...
export function AdminUsersPage() {
  ...
  const create = async (event: React.FormEvent) => {
    ...
    refresh(users.refetch, api.post('/api/users', { ...form, roleIds: form.roleId ? [form.roleId] : [] }))
  ...
  return (
    <section className="panel">
      ...
      <form className="form-grid" onSubmit={create}>
        ...
        <label>初始密码<input type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} /></label>
...
export function AdminTenantsPage() {
  ...
  const create = async (event: React.FormEvent) => {
    event.preventDefault()
    refresh(tenants.refetch, api.post('/api/tenants', form))
  ...
  return (
    <section className="panel">
      ...
      <form className="form-grid" onSubmit={create}>
        <label>租户名称<input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label>
        <label>管理员账号<input value={form.adminUsername} onChange={(e) => setForm({ ...form, adminUsername: e.target.value })} /></label>
        <label>管理员密码<input type="password" value={form.adminPassword} onChange={(e) => setForm({ ...form, adminPassword: e.target.value })} /></label>
        <label>管理员显示名<input value={form.adminDisplayName} onChange={(e) => setForm({ ...form, adminDisplayName: e.target.value })} /></label>
...
export function AdminRolesPage() {
  ...
  const create = async (event: React.FormEvent) => {
    event.preventDefault()
    refresh(roles.refetch, api.post('/api/roles', form))
    setForm({ code: '', name: '' })
  }
  return (
    <section className="panel">
      ...
      <form className="form-grid" onSubmit={create}>
        <label>角色编码<input value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} /></label>
        <label>角色名称<input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label>
